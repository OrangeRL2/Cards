...
# Existing image/cache workspace is kept so you do not have to redownload
# and rebuild every old card. The live master source is now holodori-own.
BASE = HOME / "testholo"
OWN = HOME / "holodori-own"

TOOLS = OWN / "holodori-asset-tools"
LATEST = OWN / "master-client" / "latest.json"
...
# Member names are bootstrapped from the existing organized manifest, then
# cached here. This removes the dependency on HolodoriDB Character.json.
CHARACTER_NAME_CACHE = BASE / "character-name-cache.json"
...
def require_layout() -> None:
    missing = [p for p in (TOOLS, LATEST, CATALOG) if not p.exists()]
    if missing:
...
def load_latest() -> dict:
    ...
    if not isinstance(data.get("cards"), list):
        raise SystemExit(f"Invalid latest.json: cards array missing in {LATEST}")
    return data


def load_character_names(cards: list[dict]) -> dict[str, str]:
    names: dict[str, str] = {}

    if CHARACTER_NAME_CACHE.exists():
        try:
            cached = json.loads(CHARACTER_NAME_CACHE.read_text(encoding="utf-8"))
    ...
    # Infer character_id -> member from the existing manifest.
    # Existing cards already contain both asset_id and character_id in latest.json,
    # while the manifest contains asset_id -> member.
    manifest = ORGANIZED / "manifest.csv"
    if manifest.exists():
        asset_to_character = {
            card.get("asset_id"): card.get("character_id")
            for card in cards
            if card.get("asset_id") and card.get("character_id")
        }
        ...
        with manifest.open("r", encoding="utf-8-sig", newline="") as f:
            for row in csv.DictReader(f):
                ...
                character_id = asset_to_character.get(asset_id)

                if character_id and member:
                    names.setdefault(character_id, member)

    if names:
        CHARACTER_NAME_CACHE.write_text(
...
def build_signed_cards() -> int:
    ...
    for signature_path in sorted(SIG_LAYERS.glob("mov_card_sign_*.png")):
        ...
        if destination.exists() and destination.stat().st_mtime >= max(
            signature_path.stat().st_mtime,
            card_candidates[0].stat().st_mtime,
        ):
            continue
...
def load_database() -> tuple[dict[str, str], list[dict]]:
    latest = load_latest()
    cards = latest["cards"]
    names = load_character_names(cards)
    return names, cards
...
def organize() -> tuple[int, int]:
    ...
    for data in cards:
        ...
        member = names.get(character_id)

        if not member:
            log(f"MISSING CHARACTER NAME: {character_id} ({data.get('asset_id')})")
            continue
    ...
    for row in rows:
        ...
        # Preserve the existing 001/002 naming behavior:
...
def cards_hash(cards: list[dict]) -> str:
    # Hash only card data, not latest.json's fetched_at timestamp.
    raw = json.dumps(cards, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
...
def save_state(tool_before: str, tool_after: str, master_version: str, card_hash: str) -> None:
    state = {
...
def main() -> None:
    ...
    log(f"Live master source: {LATEST}")

    previous_state = load_state()
    latest = load_latest()
    master_version = str(latest.get("master_version", "unknown"))
    card_hash = cards_hash(latest["cards"])

    tool_before, tool_after = git_update(TOOLS)
    needs_refresh, missing_ids = cards_need_refresh(previous_state, card_hash)

    log(f"Master version: {master_version}")
    ...
    if not needs_refresh:
        ...
        save_state(tool_before, tool_after, master_version, card_hash)
    ...
    if missing_ids:
        ...
        log("Card master changed. Refreshing card assets and organization.")
