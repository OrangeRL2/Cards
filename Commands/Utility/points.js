...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const cardCount = Array.isArray(u?.cards) ? u.cards.reduce((s, c) => s + (Number(c.count) || 0), 0) : 0;
      const pending = Array.isArray(u?.pendingAttempts) ? u.pendingAttempts.filter(a => !a.resolved).length : 0;
