...
const DEFAULT_RANGE = { min: 25, max: 99 }; // default daily fans range
// Special ranges for specific Discord user IDs (exact match)
const SPECIAL_USER_RANGES = {
  //MOOMOO
  '875533483051712543': { min: 1, max: 25 },
  '647219814011502607': { min: 1, max: 25 },
  '91103688415776768': { min: 1, max: 25 },
  //char
  '879614865956827197': { min: 1, max: 25 },
  //aerestria
  '953552994232852490': { min: 1, max: 25 },
  '1188023588926795827': { min: 1, max: 25 },
  '1300468334474690583': { min: 1, max: 25 },
  //blacky
  '1416081468794339479': { min: 1, max: 25 },
 ...
 * This computes the UTC instant that corresponds to 00:00 JST of the next day
 * and returns its UNIX seconds (suitable for Discord <t:...> timestamps).
 */
function nextJstMidnightUnix(now = new Date()) {
  // Build a Date that represents "now" in JST local fields
  ...
  // Date.UTC(y, m, d+1, 0, 0, 0) is the UTC ms for JST-local midnight of next day
  const nextJstMidnightUtcMs = Date.UTC(y, m, d + 1, 0, 0, 0);
    ...
    try {
      ...
      if (rec && rec.lastLoginJST === todayJST) {
        ...
        await interaction.editReply(
          `You already logged in on today. Come back after midnight (<t:${nextResetUnix}:R>.)`
        );
        return;
      }
