...
async function acquirePullLock(userId, owner, ttlMs = 120000) {
  const now = new Date();
...
async function waitForPullLock(userId, owner, timeoutMs = 20000, retryMs = 50) {
  ...
  while (true) {
    const ok = await acquirePullLock(userId, owner, 120000);
    if (ok) return true;
...
async function consumePulls(discordUserId, amount, allowEvent, specialLabel = null, options = {}) {
  ...
  if (allowEvent) {
    ...
    if ((Number(doc.eventPulls) || 0) + (Number(doc.pulls) || 0) < needed) {
      return {
        success: false,
        consumedFromEvent: 0, consumedFromTimed: 0, consumedFromSpecial: 0,
        doc, remainingEvent: doc.eventPulls ?? 0, remainingTimed: doc.pulls ?? 0,
        remainingSpecial: null, nextRefillInMs: computeNextRefill(doc)
      };
    }
  ...
  const dec = await PullQuotaModel.updateOne(
    { userId: discordUserId, pulls: { $gte: amount } },
    [
      {
        $set: {
          lastRefill: {
  ...
  if (dec && dec.modifiedCount === 1) {
    ...
    return {
      success: true,
      consumedFromEvent: 0, consumedFromTimed: amount, consumedFromSpecial: 0,
      doc: afterDoc ?? null,
      remainingEvent: afterDoc ? (afterDoc.eventPulls ?? 0) : 0,
      remainingTimed: afterDoc ? (afterDoc.pulls ?? 0) : 0,
      remainingSpecial: null,
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('multi-pull')
    .setDescription('Spend multiple pulls to draw up to 50 packs of cards.')
    .addIntegerOption(option =>
      option.setName('amount')
        .setDescription('Number of packs to pull (1-50).')
        .setMinValue(1)
        .setMaxValue(50)
        .setRequired(true)
    )
...
const frozen = isFrozen(discordUserId, member);
    ...
    let acquiredFast = await acquirePullLock(discordUserId, lockOwner, 120000);
    let queued = false;
    if (!acquiredFast) {
    ...
    try {
      ...
      // --- Draw all requested packs. Pity is evaluated sequentially for every pack. ---
      const packs = [];
      let runningSinceSEC = pullsSinceLastSEC;
      try {
        for (let packIndex = 0; packIndex < amount; packIndex += 1) {
          ...
          if (!pityNoTrack) {
            ...
            runningSinceSEC = packHasSEC ? 0 : runningSinceSEC + 1;
          }
        }
      } catch (err) {
        console.error('multi-pull draw error after consume:', err);
...
if (!SY_ANNOUNCE_EXEMPT_IDS.has(discordUserId) && String(rarity).toUpperCase() === 'SY') {
  syToAnnounce.push({ rarity: 'SY', name: displayName });
      ...
      // --- Build pack-local descriptions and show results ---
      const packDescriptions = packs.map((_, packIndex) => {
        const lines = pageItems.filter(it => it.packIndex === packIndex).map(it => it.listLine);
        return `**Pulled ${amount} pack${amount === 1 ? '' : 's'}**\n**Pack ${packIndex + 1}**\n${lines.join('\n')}`;
      });
      ...
      function makeEmbed(idx) {
        ...
        if (useSpecial) {
          return new EmbedBuilder()
            .setTitle(`Pack ${it.packIndex + 1}/${amount} • Card ${it.cardIndex + 1} **[${it.rarity}]** - ${it.displayName}${tag}`)
      ...
      const prevPackBtnEnabled = new ButtonBuilder().setCustomId('prev_pack').setLabel('◀◀ Prev Pack').setStyle(ButtonStyle.Secondary).setDisabled(amount <= 1);
      const nextPackBtnEnabled = new ButtonBuilder().setCustomId('next_pack').setLabel('Next Pack ▶▶').setStyle(ButtonStyle.Secondary).setDisabled(amount <= 1);
      const jumpPackBtnEnabled = new ButtonBuilder().setCustomId('jump_pack').setLabel('Jump Pack').setStyle(ButtonStyle.Secondary).setDisabled(amount <= 1);
      const row = new ActionRowBuilder().addComponents(prevBtnEnabled, nextBtnEnabled, prevPackBtnEnabled, nextPackBtnEnabled, jumpPackBtnEnabled);
      ...
      const prevPackBtnDisabled = new ButtonBuilder().setCustomId('prev_pack').setLabel('◀◀ Prev Pack').setStyle(ButtonStyle.Secondary).setDisabled(true);
      const nextPackBtnDisabled = new ButtonBuilder().setCustomId('next_pack').setLabel('Next Pack ▶▶').setStyle(ButtonStyle.Secondary).setDisabled(true);
      const jumpPackBtnDisabled = new ButtonBuilder().setCustomId('jump_pack').setLabel('Jump Pack').setStyle(ButtonStyle.Secondary).setDisabled(true);
      const disableRow = new ActionRowBuilder().addComponents(prevBtnDisabled, nextBtnDisabled, prevPackBtnDisabled, nextPackBtnDisabled, jumpPackBtnDisabled);
      ...
      collector.on('collect', async (btnInt) => {
        try {
          ...
          if (btnInt.customId === 'jump_pack') {
            const modalId = `jump_pack_modal_${interaction.id}`;
            const packInputId = 'pack_number';
            const modal = new ModalBuilder()
              .setCustomId(modalId)
              .setTitle(`Jump to Pack (1-${amount})`)
              .addComponents(new ActionRowBuilder().addComponents(
                new TextInputBuilder()
                  .setCustomId(packInputId)
                  .setLabel(`Pack number (1-${amount})`)
                  .setStyle(TextInputStyle.Short)
                  .setPlaceholder('Example: 2')
                  .setMinLength(1)
                  .setMaxLength(String(amount).length)
                  .setRequired(true)
              ));
            await btnInt.showModal(modal);
            const modalSubmit = await btnInt.awaitModalSubmit({
              filter: modalInt => modalInt.customId === modalId && modalInt.user.id === discordUserId,
              time: 60_000
            }).catch(() => null);
            if (!modalSubmit) return;
            const packNumber = Number(modalSubmit.fields.getTextInputValue(packInputId).trim());
            if (!Number.isInteger(packNumber) || packNumber < 1 || packNumber > amount) {
              await modalSubmit.reply({ content: `Please enter a whole pack number from 1 to ${amount}.`, ephemeral: true });
              return;
            }
      ...
      console.error('unhandled error in /multi-pull:', outerErr);
