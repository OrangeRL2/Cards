...
function addCard(userDoc, rarity, name, amount = 1) {
  ...
  if (!c) {
    c = { name, rarity, count: amount, firstAcquiredAt: now, lastAcquiredAt: now, locked: false };
...
function rollFromPool(pool) {
  const options = (pool || []).map((p) => ({
    key: `${p.rarity}||${p.image}`,
    weight: Number(p.weight || 0),
  }));

  const picked = pickWeighted(options);
  const [rarity, image] = String(picked).split('||');
  return { rarity, image };
}
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('eggschange')
    .setDescription('Eggschange eggs (card currency) for gacha, event pulls, or fans.'),
  ...
  async execute(interaction) {
    ...
    collector.on('collect', async (comp) => {
      ...
      if (isProcessing) {
        try {
          if (!comp.replied && !comp.deferred) await comp.reply({ content: 'Please wait…', ephemeral: true });
        } catch {}
        return;
      }
      ...
      try {
        ...
        if (cid.startsWith('egg_list_exchange_')) {
          ...
          try {
            ...
            try { await comp.reply({ content: 'Eggschange cancelled or timed out.', ephemeral: true }); } catch {}
          }
          return;
        }
    ...
    async function handleExchange(item) {
      ...
      try {
        ...
        try { collector.stop('exchange'); } catch {}
        ...
        try {
          await session.withTransaction(async () => {
            ...
            if (item.type === 'gacha') {
              ...
              // Store each roll with countAfter (like /pull's #)
              const rolled = [];
              ...
              await quotaDoc.save({ session });
              results = { kind: 'eventpulls', amount: amt, newTotal: quotaDoc.eventPulls };

            } else if (item.type === 'fans') {
              const amt = Number(item.amount || 0);
              userDoc.points = (userDoc.points || 0) + amt;
              results = { kind: 'fans', amount: amt, newTotal: userDoc.points };
        ...
        if (results?.kind === 'gacha') {
          ...
          // Build list of all results with line breaks + #count
          ...
          const msg = await interaction.editReply({ embeds: [makeEmbed(0)], components: [row] });
          if (!msg) return;

          if (pageItems.length <= 1) {
            setTimeout(async () => {
              try { await msg.edit({ components: [disableRow] }); } catch {}
          ...
          const pageCollector = msg.createMessageComponentCollector({
            ...
            filter: (i) => i.user.id === buyerId && (i.customId === prevId || i.customId === nextId),
          });

          pageCollector.on('collect', async (btnInt) => {
            try {
              if (btnInt.customId === prevId) pageIndex = (pageIndex - 1 + pageItems.length) % pageItems.length;
              else if (btnInt.customId === nextId) pageIndex = (pageIndex + 1) % pageItems.length;
        ...
        if (results?.kind === 'eventpulls') {
          ...
          final.addFields({ name: 'Total Fans', value: `${results.newTotal ?? (resultUser?.points ?? 0)}`, inline: true });
        }
        ...
        if (msg.startsWith('MISSING_COST::')) {
          ...
          try { await interaction.editReply({ embeds: [fail], components: [] }); } catch {}
          return;
        }
