...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(opt =>
      opt
        .setName('user')
        .setDescription('Target user to compare')
        .setRequired(true)
    )
    .addStringOption(opt =>
      opt
    ...
    .addBooleanOption(opt =>
      opt
        .setName('multi')
        .setDescription('True = only multiples, False = only singles, omitted = all')
        .setRequired(false)
    )
    .addBooleanOption(opt =>
      opt
  ...
  async execute(interaction) {
    ...
    if (!targetUser || targetUser.bot) {
      await interaction.reply({
        content: 'Please specify a valid user (not a bot).',
        ephemeral: true,
      });
      return;
    }
    ...
    try {
      ...
      // Build maps keyed by name::rarity -> count
      const keyOf = c => `${String(c.name)}::${String(c.rarity)}`;

      // Lock maps keyed by name::rarity so locked cards can be excluded by default
      const youLockedMap = new Map(
        (youDoc?.cards || []).map(c => [keyOf(c), Boolean(c.locked)])
      );

      const themLockedMap = new Map(
        (themDoc?.cards || []).map(c => [keyOf(c), Boolean(c.locked)])
      );

      const youMap = new Map(
        youCards.map(c => [keyOf(c), normCount(c.count)])
      );

      const themMap = new Map(
        themCards.map(c => [keyOf(c), normCount(c.count)])
      );
      ...
      if (mode === 'theirs-minus-yours') {
        for (const c of themCards) {
          if (!targetAllRarities && String(c.rarity).trim().toUpperCase() !== filterRNorm) {
            continue;
          }

          if (filterQ && !String(c.name).toLowerCase().includes(filterQ)) {
            continue;
          }
        ...
        for (const c of youCards) {
          ...
          const themCount = normCount(themMap.get(k));
          const youCount = normCount(c.count);
      ...
      if (results.length === 0) {
        await interaction.editReply({
          content: 'No differences found for the chosen filters.',
        });
        return;
      }
      ...
      const buildEmbed = pageIdx => {
        ...
        return new EmbedBuilder()
          ...
          .setFooter({
            text: `Page ${pageIdx + 1}/${pages.length} • ${results.length} items`,
          });
      };
      ...
      const collector = message.createMessageComponentCollector({
        ...
        filter: btn =>
          ...
          (btn.customId === prevId || btn.customId === nextId),
      });
