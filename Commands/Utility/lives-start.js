...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('startlive')
    .setDescription('Start a live attempt: pick stage, rarity, then name.')
    .addIntegerOption(o =>
      o.setName('stage')
    ...
    .addStringOption(o =>
      o.setName('rarity')
        .setDescription('Rarity to send (must be valid for chosen stage)')
        .setRequired(true)
        .setAutocomplete(true) // enable runtime filtering
   )
    .addBooleanOption(b =>
      b.setName('multi')
  ...
  async execute(interaction) {
    ...
    const chosenRarity = interaction.options.getString('rarity');
    ...
    // validate stage and rarity mapping
    if (!STAGE_ALLOWED_RARITIES[stage]) {
      return interaction.reply({ content: 'Invalid stage selected.', flags: EPHEMERAL_FLAG });
    }
    if (!STAGE_ALLOWED_RARITIES[stage].includes(chosenRarity)) {
      return interaction.reply({
        content: `Invalid rarity for selected stage. Allowed rarities for ${stageName}: ${STAGE_ALLOWED_RARITIES[stage].join(', ')}`,
        flags: EPHEMERAL_FLAG
      });
    }

    // If anyRandom is true, pick a random matching owned card and confirm
    if (anyRandom) {
      try {
        await interaction.deferReply({ flags: EPHEMERAL_FLAG });
      } catch (err) {
        try { await interaction.reply({ content: 'Processing...', flags: EPHEMERAL_FLAG }); } catch {}
      }

      // reload user just before picking
      const user = await User.findOne({ id: userId }).lean();
      if (!user || !Array.isArray(user.cards) || user.cards.length === 0) {
        try { await interaction.editReply({ content: "You have no cards to send.", embeds: [], components: [] }); } catch {}
        return;
      }
      ...
      if (!candidate) {
        ...
        try { await interaction.editReply({ content: msg, embeds: [], components: [] }); } catch {}
        return;
      }
      ...
      try {
        await interaction.editReply({ embeds: [confirmEmbed], components: [confirmRow], content: null });
      } catch {
        try { await interaction.followUp({ embeds: [confirmEmbed], components: [confirmRow], flags: EPHEMERAL_FLAG }); } catch {}
      }

      const replyMsg = await interaction.fetchReply();
      const btnCollector = replyMsg.createMessageComponentCollector({ time: 30_000 });
      ...
      btnCollector.on('collect', async btn => {
        ...
        if (btn.customId.startsWith('live_confirm')) {
          ...
          // persist last options for this stage (multi/any/rarity)
          try {
            await User.updateOne(
              { id: userId },
              { $set: { [`lastLiveOptions.stage_${stage}`]: { multi: multiOnly, any: anyRandom, rarity: chosenRarity } } }
            ).exec();
          } catch (e) {
            console.warn('[live.start] failed to persist lastLiveOptions', e);
          }
    ...
    // Normal flow: show modal to ask for name and proceed
    ...
    try {
      ...
      try { await interaction.reply({ content: 'Could not open name modal. Try again.', flags: EPHEMERAL_FLAG }); } catch {}
      return;
    }
    ...
    try {
      ...
      try { await interaction.followUp({ content: 'No response from modal (timed out).', flags: EPHEMERAL_FLAG }); } catch {}
      return;
    }
